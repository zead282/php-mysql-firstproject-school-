...
$id=$_GET['cid'];
if(isset($_POST["course-short"]) && isset($_POST["course-full"]))
{
	if(!empty($_POST["course-short"]) && !empty($_POST["course-full"]))
	{ 
		$course=new Course();
...
//$q=new Database();
		...
		$course->cd=$_POST["udate"];
		$course->cid=$id;
		if($auth->editcources($course))
		{
...
<form method="post" >
  ...
  <input class="form-control" name="course-short" id="cshort"  value="" required="required"  >       
		...
		<div class="col-lg-6">
<input class="form-control" name="course-full" id="cfull" value="" required="required" >         
